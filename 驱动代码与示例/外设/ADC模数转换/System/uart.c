 ...
 *
 * 【NVIC Settings】
 *   └─ USART1 global interrupt 打勾
 *
 * 【DMA Settings（仅 UART_RX_MODE_DMA 模式需要）】
 *   ├─ Add → USART1_RX
 ...
 *
 * 【接线说明（STM32F103C8T6）】
 *   ├─ PA9  → TX（接 USB 转串口模块 RX）
 *   └─ PA10 → RX（接 USB 转串口模块 TX）
 *
 * 【接收模式切换（uart.h 配置区）】
 *   ├─ #define UART_RX_MODE_DMA  → 使用 DMA 接收，需配置上方 DMA Settings
 *   └─ #define UART_RX_MODE_IT   → 使用中断接收，无需 DMA，帧结束判断为 \n
 *
 * ───────────────────────────────────────────────────────────────────────────*/