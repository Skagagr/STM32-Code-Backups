...
static ESP8266_Bus_Ctx esp8266_ctx;
...
int main(void)
{
    ...
    int error_esp8266;
    error_esp8266 = ESP8266_Init(&esp8266_ctx, &huart1);
    ...
    while (1)
    {
        ...
        if (error_esp8266 == 0)
            OLED_ShowString(&oled_ctx, 0, 0, "Error");
        else if (error_esp8266 == 1)
            OLED_ShowString(&oled_ctx, 0, 0, "OkOk");

        
        if (error_mqtt == 1)
            OLED_ShowString(&oled_ctx, 64, 48, "OkOk");
        else
            OLED_ShowString(&oled_ctx, 64, 48, "Error");
